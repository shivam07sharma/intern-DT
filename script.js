...
document.getElementsByClassName("grey-box")[0].innerHTML=`<h2>${data.tasks[0].task_title}</h2>
<p>${data.tasks[0].task_description}</p>`

data.tasks[0].assets.forEach(element => {
    document.getElementById("asset-cont").innerHTML += `<div class="asset">
                <div class="heading"><span class="insideheading">${element.asset_title}</span><span
                    class="material-symbols-outlined iLogo">
                   home
                </span>
            </div>
            <div class="desc"><h4>Description: </h4><div>${element.asset_description}</div></div>
            <iframe width="100%" height="300" src="${element.asset_content}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
            </div>`
        });
